...
export function QueueHealth({ stats }: QueueHealthProps) {
  const getHealthStatus = (queue: QueueHealthProps["stats"]["whatsapp"]) => {
    if (queue.failed > 0) return { status: "warning", label: "Issues", icon: AlertTriangle, color: "text-yellow-600" };
    if (queue.active > 0 || queue.waiting > 0) return { status: "active", label: "Active", icon: Activity, color: "text-blue-600" };
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between p-3 rounded-lg border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium">WhatsApp Queue</p>
              <p className="text-sm text-muted-foreground">
                {stats.whatsapp.waiting} waiting, {stats.whatsapp.active} active
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between p-3 rounded-lg border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {stats.email.waiting} waiting, {stats.email.active} active
              </p>
            </div>
          </div>
